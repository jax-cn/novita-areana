...
This guide explains how to use the features provided by the Captured Surface Control API to control a display surface (browser tab, window, or screen) captured by the [Screen Capture API](https://developer.mozilla.org/en-US/docs/Web/API/Screen_Capture_API).
...
This functionality is all accessed via the [`CaptureController`](https://developer.mozilla.org/en-US/docs/Web/API/CaptureController) object. To control a captured display surface, a capture controller must be passed into a [`MediaDevices.getDisplayMedia()`](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getDisplayMedia) call inside its options object:
...
If permission is allowed by the website policy, the user may then grant (or deny) permission to access the controlled APIs. This can either be explicit permission, granted by responding to a prompt, or implicit permission granted by interacting with a control that calls one of the methods ( [transient activation](https://developer.mozilla.org/en-US/docs/Glossary/Transient_activation)) when user permission has not been explicitly denied.
...
This contains two sets of [`<button>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/button) elements — one to start and stop screen capture, and one to control zooming the captured display surface. The latter also includes an [`<output>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/output) element to print the current zoom level into.
...
To finish off our function, we call [`CaptureController.setFocusBehavior()`](https://developer.mozilla.org/en-US/docs/Web/API/CaptureController/setFocusBehavior) to stop the focus shifting to the captured display surface when the capture starts, and call our user-defined `startForwarding()` function to enable scrolling the captured display surface with wheel/touchpad gestures. We'll explain this function later on.
...
- "Zoom out" button: `decreaseZoom()`. This calls the [`CaptureController.decreaseZoomLevel()`](https://developer.mozilla.org/en-US/docs/Web/API/CaptureController/decreaseZoomLevel) method, zooming the captured surface out.
- "Zoom in" button: `increaseZoom()`. This calls the [`CaptureController.increaseZoomLevel()`](https://developer.mozilla.org/en-US/docs/Web/API/CaptureController/increaseZoomLevel) method, zooming the captured surface in.
- "Reset zoom" button: `resetZoom()`. This calls the [`CaptureController.resetZoomLevel()`](https://developer.mozilla.org/en-US/docs/Web/API/CaptureController/resetZoomLevel) method, resetting the captured surface to its starting zoom factor, which is `100`.
...
# Captured Surface Control API demo

Start CaptureStop Capture

Zoom -100%Zoom +Reset zoom
